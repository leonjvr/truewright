...
//! (agent-harness spec: verified against a real Chrome + a real local
//! HTTP LLM stub, not an in-process mock -- same discipline as
//! `crates/engine/tests/support/http_server.rs` and
//! `crates/llm/tests/support/stub_server.rs`, neither of which this crate
//! can import directly since they're each private to their own crate's
//! `tests/` directory). Returns a scripted FIFO sequence of full
//! chat-completion response bodies -- one per request received, repeating
//! the last entry once exhausted -- and records every request body for
//! assertions.
