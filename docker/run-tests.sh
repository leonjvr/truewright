#!/usr/bin/env bash
# Build the truewright test image and run the full workspace test suite plus
# `truewright doctor` inside a disposable container. The repo is mounted read-write
...
docker build -t truewright-test -f docker/Dockerfile .

docker run --rm \
  -v "$(pwd)":/app \
  -v truewright-cargo-registry:/usr/local/cargo/registry \
  -v truewright-container-target:/app/target \
  -w /app \
  truewright-test \
