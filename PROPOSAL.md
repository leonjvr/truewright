# Truewright (`truewright`) — Proposal: An LLM-First Browser-Testing Engine
...
**`truewright`: one static Rust binary** with subcommands `daemon | mcp | run | trace | doctor`.

```
┌─────────────┐  stdio MCP   ┌──────────────────────────────┐   CDP (one WebSocket,
│ Claude Code ├─────────────►│ truewright daemon (~15–30 MB RSS)   │   flatten sessions)
└─────────────┘  (truewright mcp    │  ├ session registry           ├──────────────► installed
...
│ truewright run *.yaml ────────────►  └ trace writers              │    mode)
└─────────────┘              └──────────────────────────────┘
```
...
- **Attach to the installed browser.** Registry lookup (`App Paths\chrome.exe` / `msedge.exe`) with well-known-path fallback; launch with `--remote-debugging-port` and a **dedicated `--user-data-dir`** under `%LOCALAPPDATA%\truewright\profiles\` — never the user's live profile.
...
| | Playwright + playwright-mcp | **truewright** |
|---|---|---|
...
- **Trace:** JSONL of `{ts, kind: action|snapshot|console|network|screenshot|assert, seed, persona, …}` with screenshots as files; `truewright trace export` renders a static HTML viewer (later phase).
...
**One `browser` MCP server** (agents handle one coherent toolset better), built on the official **`rmcp`** Rust SDK. Transports: stdio (`truewright mcp` shim auto-spawns the daemon) and streamable HTTP on loopback with a bearer token.
...
Runner mode (`truewright run tests/*.yaml`) executes YAML specs whose steps map **1:1 to the MCP tool names/args**. Because the mapping is exact, an agent's exploration trace exports directly as a runnable regression test:
...
| | Puppeteer | Playwright | playwright-mcp | **truewright** |
|---|---|---|---|---|
...
truewright/
  ...
  src/main.rs              # truewright CLI: daemon | mcp | run | trace | doctor
```
...
|---|---|---|---|
| **0 — CDP spike** ✅ | Hand-rolled client: attach to installed Chrome *and* Edge, create context, navigate, `Runtime.evaluate`, screenshot | 1–2 wks | `truewright doctor` passes on both browsers; command round-trip p50 < 5 ms |
...
| **5e — Hardening (HTML trace viewer)** ✅ | The originally-envisioned CLI surface (`daemon \| mcp \| run \| trace \| doctor`) named `trace` as a first-class subcommand; only `doctor`/`mcp` existed before this. `truewright trace view <name>` and `browser_render_trace(name)` render an already-saved trace as one self-contained HTML file. The one schema addition -- a `Screenshot` trace entry -- was deliberately narrow: network/DOM-snapshot entry types were considered and dropped as redundant (network-mocking already covers traffic) or not more useful as HTML than the existing text already is; screenshots earn their place since nothing else in a trace can show what the page looked like. Screenshots embed as base64 data URIs in the rendered HTML (self-contained, portable) but stay path-referenced in the JSONL trace itself (keeps it lightweight/greppable) | 3 hours | Verified: a captured trace with console/exception/action/screenshot entries renders to HTML with everything in chronological order and the screenshot visible inline; manually confirmed legible by publishing the generated output as a private artifact. Suite green on host and in the Docker container |
| **5f — Hardening (weekly Chrome-beta CI)** ✅ (partially observed) | Browser discovery gains `TRUEWRIGHT_CHROME_PATH`, an explicit-binary override that wins over both normal registry/well-known-path discovery and the managed `chrome-headless-shell` (the latter matters because headless is the default for the test suite this exists to redirect -- an override that only affected headed launches would be silently ignored by nearly every test). Backs a second Docker image (`docker/Dockerfile.chrome-beta`, real `google-chrome-beta` from Google's own apt repo) and a weekly-scheduled GitHub Actions workflow running the full suite against it, to catch CDP protocol drift before it reaches Stable | 2 hours | Live-verified: the override correctly redirects both plain discovery and the headless-shell path to a forced binary; `docker/Dockerfile.chrome-beta` really builds and installs `google-chrome-beta`; `cargo test --workspace` really passes running against it inside that container. **Not verified:** the GitHub Actions workflow's actual execution -- this repo has no git remote yet, so it's hand-written and locally syntax-checked (parsed with a local YAML parser) but has never been observed running on GitHub's own schedule/runners |
| **5g — Hardening (cross-origin OOPIF)** ✅ | A cross-origin `<iframe>` Chrome has isolated into its own process (an OOPIF -- a real separate CDP target, common for third-party payment/auth widgets) now shows its actual content instead of the "not inspectable" leaf `same-origin-iframes` left it at. Discovery/attach reuses the already-proven browser-wide discover-and-explicit-attach mechanism, not `Target.setAutoAttach` (settled by `popup-auto-attach`'s own hang bug). New `DOM.getFrameOwner`/`DOM.resolveNode`/`Runtime.callFunctionOn` CDP surface correlates an OOPIF target back to the exact `<iframe>` element the walker assigns a ref to (every iframe now gets one, not just same-origin ones), invoking the *same* `refFor` registry logic the walker itself uses so there's no separate ref-allocation scheme to drift out of sync. OOPIF content is spliced in with namespaced refs (`f1:e3`); click/type coordinate translation composes two already-existing, *unmodified* pieces (`resolve.js` run inside the OOPIF's own session, plus its on-screen position via its own correlated top-level ref) rather than needing new script logic | 1 day | Live-verified against a real forced OOPIF (two distinct `.localhost` subdomains -- different registrable domains/sites, unlike same-`127.0.0.1`-different-port -- launched with `--site-per-process`): the OOPIF's real content appears in the snapshot with a namespaced ref, and clicking it actually fires its own click handler, confirmed on host and in the Docker container. One real bug found and fixed: the original plan to scope correlation via `Page.getFrameTree` doesn't work (it only reports frames within its own target's renderer process, so a genuine cross-process OOPIF is invisible to it by construction) -- fixed by relying on `DOM.getFrameOwner` succeeding as the scope check instead, one mechanism instead of two, and the now-dead `Page.getFrameTree` protocol code was removed rather than left unused |
| **5h — Hardening (BiDi spike)** ✅ (research, no shipped code) | Investigated whether WebDriver BiDi is a viable alternative wire protocol, and what it would buy. Two live tests, not literature recall alone: (1) `POST /session` against a real launched Chrome's debugging port returned `404` -- Chrome's own server has no WebDriver/BiDi endpoint; (2) `puppeteer-core` launched with `protocol: 'webDriverBiDi'` against the same Chrome, with its stdout captured, showed the browser running in ordinary CDP mode throughout -- Puppeteer's bundled `chromium-bidi` "Mapper" does the BiDi↔CDP translation client-side, not the browser | 1 day | **Finding: Chrome doesn't natively speak BiDi -- it's CDP with a client-side translation layer bolted on, for Chromium browsers specifically.** Building BiDi support for Chrome would mean owning *more* protocol code (an equivalent translation layer), not less. BiDi's one genuine non-CDP use case is Firefox (native BiDi, no CDP at all) -- not independently verified here (no Firefox/geckodriver on this machine, and installing a browser wasn't in scope for a spike). **Recommendation: don't build BiDi now** -- Firefox support isn't a current goal, and the only Chrome-facing benefit hoped for doesn't hold up. Architecture read for if that changes: `crates/cdp/src/transport.rs`'s `Transport` trait is CDP-agnostic and reusable as-is; the CDP envelope + flatten-`sessionId` model (`connection.rs`, `protocol/target.rs`) is real, contained friction with no BiDi equivalent; `engine/src/session.rs`'s action-dispatch logic is already mostly protocol-capability-shaped. Full write-up: `.research/bidi-protocol-spike.md` (this is a research spike, not a shippable feature -- no OpenSpec change, matching this project's `.research/REVIEW.md` precedent for non-shipping research) |
| **5 — Hardening (remaining)** | none currently identified -- Phase 5's originally-scoped hardening items are all now shipped or spiked | done | — |
| **6a — Agent harness (LLM providers)** ✅ | `truewright` gains its own LLM agent harness across four sequential changes -- this is the first: a provider-agnostic client, not the driving loop itself (that's 6c). New `crates/llm`: wire-neutral chat types with zero provider-specific serde shape (so the Responses-API shape 6b adds never touches them); `CompatClient` speaks the one OpenAI-compatible `/chat/completions` wire shape DeepSeek/MiniMax/GLM/Grok/OpenAI all share (text + image content, function-calling tools, 429/5xx retry); `<data-dir>/truewright/config.toml` (`[providers.*]`/`[roles.*]`, four-way path resolution, missing file = valid empty config so browser-only tools never break); `CredentialSource` resolves a literal/env-var key now, structured so 6b's OAuth variant adds zero call-site changes. `truewright llm ping <role>` is the live-verification hook and a standalone connectivity check | 1 day | Live-verified against a real local HTTP server (`crates/llm/tests/support/stub_server.rs`, a new body-reading/header-recording test server): real socket round trip, Authorization header, 5xx-then-success retry, non-retryable 4xx surfaces immediately. Plus a manual end-to-end CLI check (`truewright llm ping driver` against a real local Python stub) confirming the full CLI→config→role-resolution→HTTP→output path, not just the library in isolation. One real bug found via this module's own unit tests, not assumed: `#[serde(rename_all = "kebab-case")]` on `ProviderKind` produced `open-ai-compat` instead of `openai-compat` (serde treats "OpenAi" as two word-boundary-split words) -- fixed with explicit per-variant `#[serde(rename)]`. Suite green on host and in the Docker container |
| **6b — Agent harness (OAuth/subscription auth)** ✅ (backend not independently verified) | PKCE flow (`crates/llm/src/auth/`, was a single `auth.rs` file, now a directory) + hand-rolled one-shot localhost callback listener, token store (`chmod 600` on Unix), transparent refresh, `truewright auth login/status/logout`; `ResponsesClient` (SSE-aggregated) for ChatGPT-subscription usage. Every OpenAI endpoint/header/client-id constant verified directly against `openai/codex`'s own source, not third-party writeups -- caught two real, easy-to-miss details: the code exchange is form-encoded but refresh is JSON-encoded, and expiry comes from the `id_token`'s own `exp` claim (no separate `expires_in` field in the response). Opt-in, clearly labelled as unofficial/ToS-gray-area; `truewright` sends its own `originator` header, not Codex's, rather than misrepresenting its client identity | 1 day | Every HTTP-shaped piece verified against a real local server/socket (not hand-built JSON fed to a parser): form-vs-JSON exchange/refresh with real JWT account-id/expiry extraction, a real callback listener accepting a real HTTP GET (including the denied-consent path), and `ResponsesClient` aggregating a genuine `text/event-stream` byte stream for both text and tool-call replies, with the account-id header confirmed present on a real outgoing request. **Not independently verified:** the real `auth.openai.com`/`chatgpt.com` endpoints -- needs a human clicking through OpenAI's real consent screen with a real subscription, deliberately left as a manual, user-assisted step. **One real design bug found and fixed before shipping:** stored tokens were looked up by the config's provider name while login/refresh saved them keyed by the flow's own id -- silently broken whenever they differ, invisible to every test until one was deliberately written with mismatched names. Suite green on host and in the Docker container |
| **6c — Agent harness (the driving loop)** ✅ | New `crates/agent`: the actual tool-calling loop over `engine::Session` (navigate/click/type/snapshot/assert/screenshot/...), context pruning, capability-flag-routed vision (screenshots go to a `vision` role when the `driver` role lacks vision), skills. `truewright agent "task"` CLI, one-shot with live step progress. Recoverable per-action failures (stale ref, failed assertion, wait timeout) are fed back to the model as tool-result text and the loop continues, rather than aborting the run -- adapting to that is the entire point of a loop over a fixed script. Two consecutive no-tool-call turns (after one nudge), a step budget, or a task timeout each end the run with a clear, specific outcome instead of hanging | 1 day | Live-verified against a real Chrome session and real local stub servers (`crates/agent/tests/support/llm_stub.rs`, a new scripted OpenAI-compatible `/chat/completions` stub): a full navigate/type/click/assert/task_complete sequence passes; a deliberately-stale ref surfaces as tool-result text and the model recovers on the next step; older page snapshots are elided once past the retention limit while the most recent ones and every non-snapshot result stay intact; both vision-routing paths (inline image for a vision-capable driver, routed-and-interpreted text for a non-vision one); no-progress and max-steps termination. Plus a manual end-to-end `truewright agent` CLI smoke test against a real separate-process stub and real headless Chrome, human-readable and `--json` output, exit code 0 on pass. Suite green on host and in the Docker container |
| **6d — Agent harness (MCP delegation)** ✅ | Fourth and last of the agent-harness changes. `browser_run_task(task, guidance?, skills?, max_steps?)` -- an outer MCP host model delegates a whole sub-task to truewright's own configured driver, running over the *same* `engine::Session` every other `browser_*` tool already shares (not a second, disconnected session); a failed task is a tool-level error carrying the step transcript, like `browser_assert`. `browser_screenshot`/`browser_record_stop` gain optional `interpret`/`guidance` params routing the image through the configured vision role for text instead, purely additive (omitted = today's exact behavior). `TruewrightTools` gains an optional `agent: Option<AgentConfig>` via a new `.with_agent(...)` builder rather than a constructor parameter, so every existing constructor/call site/test is untouched by a field that's `None` in the common case; `Client`/`CompatClient`/`ResponsesClient`/`RoleClient`/`Harness` gain `#[derive(Clone)]` so a per-call `Harness` (MCP's own lower default `max_steps`, 25) can be built without threading a new parameter through `Harness::run_task` or touching change 3's existing call sites/tests. `truewright mcp` gains `--config`; a config parse failure warns to stderr, while simply having no `[roles.driver]` stays silent, the same zero-LLM-setup guarantee every other `truewright` subcommand already makes | 1 day | Live-verified against a real Chrome session and real local LLM stubs (new `tests/support/llm_stub.rs` at the workspace-binary level) over a real streamable-HTTP MCP client, extending the existing `tests/mcp_http_flow.rs` pattern: a full `browser_run_task` pass against a real fixture page; the session surviving a delegated task for further ordinary `browser_*` calls afterward; no driver configured failing as a clear protocol-level error; `browser_screenshot(interpret: true)` returning a real second vision stub's text, not an image block. Plus a manual end-to-end smoke test of the actual compiled `truewright mcp --http` binary against a real separate-process Python LLM stub and real headless Chrome, driven by raw `curl` MCP requests -- confirmed the exact transcript+PASS output shape over the wire, not just through the Rust test harness. Suite green on host and in the Docker container (one unrelated pre-existing flake in `true_input_flow`'s real-OS-dispatch test during the full parallel run, confirmed to pass in isolation and untouched by this change) |
