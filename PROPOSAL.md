...
| **2.5 — Human motion (trained)** | Explicit training mode: record a real human's DOM-trusted input events once, fit a `Persona` from the observed variability, replay a freshly re-sampled (never byte-identical) approximation; replaying an untrained profile fails clearly instead of silently falling back to a synthetic persona | 1–2 wks | A trained profile's replay is visibly distinct per run yet statistically consistent with the recording; requesting replay of a never-trained name returns a typed "training required" error |
