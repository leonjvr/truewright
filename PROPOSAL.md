...
| **3b — Determinism (remaining primitives)** | Init scripts, both clock modes, seeded randomness, console capture, JSONL traces, `browser_assert`, YAML runner + trace→YAML export | 2–3 wks | A recorded agent exploration replays green 20/20 against mocked network + deterministic clock/randomness; a 15-test suite runs headless in CI |
