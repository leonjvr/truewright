...
| **6d — Agent harness (MCP delegation)** | `browser_run_task(task, guidance?)` -- an outer MCP host model delegates a whole sub-task to aib's own configured driver; `browser_screenshot`/`browser_record_stop` gain an `interpret` mode returning vision-model text for vision-less outer models | pending | — |
