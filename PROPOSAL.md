...
| **3b-iv — Tooling (unified action trace, assertions, runner)** | A unified action trace merging navigate/click/type calls into the same JSONL stream console-capture introduced, `browser_assert`, YAML runner + trace→YAML export | 1–2 wks | A recorded agent exploration replays green 20/20 against mocked network + deterministic clock/randomness; a 15-test suite runs headless in CI |
