...
| **6b — Agent harness (OAuth/subscription auth)** | PKCE flow + one-shot localhost callback, token store, refresh, `aib auth login/status/logout`; `openai-responses` client (SSE-aggregated) for ChatGPT-subscription usage, opt-in and clearly labelled as unofficial/ToS-gray-area | pending | — |
