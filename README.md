...
**Why:** Playwright/Puppeteer are resource-heavy (Node driver + ~1 GB bundled browsers + browser-per-worker) and their input doesn't mimic a real user (teleporting mouse, instant clicks). `truewright` fixes both while keeping deterministic testing.
...
- **Phase 1 (agent MVP) — done.** `crates/engine` adds a session layer with an injected DOM/ARIA walker (token-efficient, ref-addressable snapshots), bounded-poll actionability, and click/type/press/wait_for/screenshot. `crates/mcp` + `truewright mcp` expose it all as a stdio MCP server (`browser_navigate`, `browser_snapshot`, `browser_click`, `browser_type`, `browser_press`, `browser_wait_for`, `browser_screenshot`, `browser_close`), verified end-to-end against a real page. This is a scoped-down slice of PROPOSAL.md's full vision — see `openspec/changes/archive/*-phase-1-agent-mvp/design.md` for exactly what's deferred (isolated-world injection, MutationObserver-driven actionability, human motion, multi-session daemon).
- **Browser efficiency — done.** Headless launches get memory-reduction flags, Playwright's automation-default flag set (to shed background service processes), and an auto-downloaded, cached `chrome-headless-shell` (installed-browser fallback; `--browser installed` opt-out); `truewright doctor` reports full process-tree memory per browser. Measured: headless-shell ~170–350 MB vs installed Chrome/Edge headless ~450–1550 MB for the same page, on both Windows and Linux. On Unix the CDP transport is `--remote-debugging-pipe` (fds 3/4, no TCP port) — lighter and free of the attach-timeout flakiness a loopback port scan hit in sandboxed containers.
...
- **Streamable-HTTP MCP transport (Phase 5) — done.** `truewright mcp --http` serves the identical tool surface stdio does over a loopback-only HTTP listener guarded by a bearer token, for MCP clients that connect over HTTP instead of spawning `truewright` as a subprocess. Always binds `127.0.0.1` — no flag exposes it beyond loopback. Each HTTP session gets its own independent browser (a uniquely-suffixed profile directory), so concurrent sessions never collide. Verified: real requests with no/wrong token get `401`; a correctly authenticated session completes the MCP handshake and lists the real tool set; two concurrent sessions each navigate a real browser without a profile-directory collision — confirmed both via the automated suite and manually against the compiled binary with `curl`.
- **Same-origin iframes (Phase 5) — done.** `browser_snapshot` now sees into same-origin `<iframe>` content instead of silently omitting it — real-world apps embed same-origin iframes constantly (rich-text editors, embedded widgets, app-internal micro-frontends), and previously the walker showed nothing where one existed. Nested content appears under an `iframe` entry with usable refs, and `browser_click`/`browser_type` on those refs land at the correct on-screen coordinates (accounting for the iframe's own position and any ancestor iframes' positions). A cross-origin iframe with no attachable OOPIF target (see below) is shown as an explicit "not inspectable" leaf rather than vanishing. Verified: a fixture with both a same-origin (`srcdoc`) and cross-origin (`data:` URL) iframe shows the same-origin button in the snapshot, clicking it via its ref actually fires the iframe's own click handler (proving the coordinate math, not just that the CDP call didn't error), and the cross-origin frame renders as the explicit boundary text.
...
- **HTML trace viewer (Phase 5) — done.** `truewright trace view <name>` and the `browser_render_trace(name)` MCP tool render an already-saved trace as one self-contained HTML file — console/exception/action entries in chronological order, color-coded by kind. `browser_screenshot` calls made while a trace is active are now captured into it too (a new trace entry kind, the one thing a text-only trace could never show) and appear inline in the rendered page as embedded images, not external file references — the HTML file needs nothing alongside it to be readable. Verified: a captured trace with console/exception/action/screenshot entries renders correctly, with the screenshot visible inline as a base64-embedded image; visually confirmed legible.
- **Weekly Chrome-beta CI (Phase 5) — done, partially observed.** Browser discovery honors a new `TRUEWRIGHT_CHROME_PATH` override (bypassing both normal discovery and the managed `chrome-headless-shell`, for headless runs too), backing a second Docker image (`docker/Dockerfile.chrome-beta`) that installs real Google Chrome Beta and a weekly-scheduled GitHub Actions workflow (`.github/workflows/chrome-beta.yml`) that runs the full suite against it, to catch CDP protocol drift before it reaches Stable. The override and the Chrome-Beta Docker run are live-verified (the image really installs `google-chrome-beta`, and `cargo test --workspace` really passes against it); the GitHub Actions workflow itself is hand-written and locally syntax-checked but **not observed actually running** — this repository has no git remote yet, so there's nowhere to push it and watch it execute.
...
- **LLM providers (Phase 6, agent harness 1/4) — done.** New `crates/llm`: a provider-agnostic OpenAI-compatible chat-completions client (text/image content, function-calling tools, 429/5xx retry) that DeepSeek/MiniMax/GLM/Grok/OpenAI all speak identically, plus `<data-dir>/truewright/config.toml` (`[providers.*]`/`[roles.*]`, four-way path resolution, a missing file is a valid empty config so browser-only tools never break). `truewright llm ping <role>` resolves a configured role and sends one real completion — the live-verification hook, and a standalone connectivity check. First of four changes building `truewright`'s own agent harness (multi-provider LLM driving its own browser session); OAuth/subscription auth, the driving loop, and MCP delegation are the next three. Verified against a real local HTTP server (not just serialized-shape assertions) for the retry/auth-header behavior, plus a manual end-to-end CLI smoke test against a real local stub.
- **OAuth/subscription auth (Phase 6, agent harness 2/4) — done, backend not independently verified.** `truewright auth login <flow>` / `status` / `logout` support subscription-based providers alongside plain API keys — currently OpenAI's ChatGPT Plus/Pro/Team/Enterprise subscription, via the same PKCE authorization-code flow (and the same `chatgpt.com/backend-api/codex` Responses-API backend) OpenAI's own open-source Codex CLI uses. Every endpoint/header/client-id constant was verified directly against `openai/codex`'s own source, not third-party writeups — which caught two easy-to-miss real details: the initial code exchange is form-encoded but refresh is JSON-encoded, and token expiry comes from the `id_token` JWT's own `exp` claim (the token endpoint's response has no separate `expires_in` field). A real bug was found and fixed before shipping: stored tokens were looked up by the config's provider name instead of the OAuth flow's own id, silently broken whenever they weren't identical — caught by a test deliberately using different names for the two, since every other test's coincidental name-matching hid it. Every HTTP-shaped piece (token exchange/refresh, the local callback listener, SSE response aggregation) is verified against a real local server or socket, not hand-built JSON fed straight to a parser. **What's not independently verified:** the real `auth.openai.com`/`chatgpt.com` endpoints — that needs a human clicking through OpenAI's real consent screen with a real subscription, deliberately left as a manual, user-assisted step rather than assumed to work.
- **Agent harness (Phase 6, agent harness 3/4) — done.** `truewright agent "task"` runs a configured LLM role as its own driver against `truewright`'s browser session, autonomously — the core deliverable of the four-part agent-harness effort. New `crates/agent`: a small tool surface (navigate/snapshot/click/type/press/wait_for/assert/screenshot/list_pages/switch_page/run_yaml, plus task_complete/task_failed) executed directly against `engine::Session`; recoverable action failures (a stale ref, a failed assertion, a timeout) come back to the model as feedback text instead of aborting the run, since adapting to that is the entire point of a loop over a fixed script. Context pruning keeps only the most recent snapshots in play so a weak-context driver doesn't drown in stale page state. Screenshot routing follows the driver role's own `vision` flag — inline for a vision-capable driver, sent to a configured `[roles.vision]` for text interpretation otherwise, a clear error if neither applies. Skills (reusable Markdown guidance, `--skill name`) attach to the system prompt. `--driver`/`--vision <provider>/<model>` override `[roles.*]` entirely for one-off runs. Verified against a real Chrome session and real local HTTP stubs (8 integration tests: full task pass, error-feedback recovery, context pruning across distinctly-marked pages, termination on no-progress/step-budget, both vision-routing paths), plus a manual end-to-end CLI smoke test (separate-process stub, real headless Chrome, human-readable and `--json` output).
- **MCP task delegation (Phase 6, agent harness 4/4) — done.** The `browser` MCP server itself can now delegate to `truewright`'s own driver -- the fourth and last change of the agent-harness effort. New `browser_run_task(task, guidance?, skills?, max_steps?)` tool runs the configured driver's agent loop over the *same* `engine::Session` every other `browser_*` tool on the server already shares, so an outer MCP host (e.g. Claude, or any other agent host with `truewright` configured as an MCP server) can hand off a whole sub-task and keep driving the same live browser afterward -- not two disconnected sessions. `browser_screenshot`/`browser_record_stop`'s preview frame gain optional `interpret`/`guidance` params that route the image through the configured vision role and return text instead, for a host model with no vision of its own; omitting them keeps returning the image exactly as before. `TruewrightTools` gains an optional `agent: Option<AgentConfig>` (`None` whenever `[roles.driver]` isn't configured) via a new `.with_agent(...)` builder, so every existing constructor and browser-only tool is untouched. `truewright mcp` gains `--config`. A failed delegated task is a tool-level error carrying the step transcript, matching how `browser_assert` already reports failure -- not a crash, not a silent no-op. Verified against a real Chrome session and real local LLM stubs over a real streamable-HTTP MCP client (a full `browser_run_task` pass, no-driver-configured erroring clearly, screenshot interpretation returning text not an image, the session surviving a delegated task for further ordinary tool calls afterward), plus a manual end-to-end smoke test of the actual compiled `truewright mcp --http` binary against a real separate-process Python stub and real headless Chrome, driven by raw `curl` MCP requests. This completes the four-part agent harness: `truewright` can now drive its own browser autonomously from the CLI (`truewright agent`) or be driven by an outer MCP host that delegates sub-tasks to it, with any OpenAI-compatible provider (or a ChatGPT subscription via OAuth) and multi-model vision routing.

## MCP server

Configure `truewright mcp` as a stdio MCP server in an agent host (e.g. Claude Code, Claude Desktop), or run `truewright mcp --http` to serve over loopback HTTP instead (see below). It lazily launches a browser on the first tool call and exposes: `browser_navigate(url)`, `browser_snapshot()`, `browser_click(ref, human_like?, persona?, trained_profile?, seed?, true_input?)`, `browser_right_click(ref, human_like?, persona?, trained_profile?, seed?, true_input?)`, `browser_type(ref, text, submit?, human_like?, persona?, trained_profile?, seed?, true_input?)`, `browser_press(key)`, `browser_wait_for(text, timeout_ms?)`, `browser_screenshot()`, `browser_record_start(max_duration_ms?, quality?)`, `browser_record_stop()`, `browser_train_start(name)`, `browser_train_stop()`, `browser_network_record_start(name)`, `browser_network_record_stop()`, `browser_network_replay_start(name)`, `browser_network_replay_stop()`, `browser_add_init_script(source)`, `browser_seed_randomness(seed)`, `browser_set_clock(time_ms)`, `browser_advance_clock(ms)`, `browser_console_start(name)`, `browser_console_stop()`, `browser_assert(text, present?)`, `browser_run_yaml(source)`, `browser_export_yaml(name)`, `browser_render_trace(name)`, `browser_list_pages()`, `browser_switch_page(page_id)`, `browser_close()`. Refs come from the snapshot text (e.g. `[e6]`).

**Streamable-HTTP transport:**

```
truewright mcp --http                              # listens on http://127.0.0.1:8787/mcp, prints a random bearer token
truewright mcp --http --port 9000 --token my-secret  # explicit port + token instead of a random one
```

Point an MCP-HTTP-capable client at `http://127.0.0.1:<port>/mcp` with `Authorization: Bearer <token>`. Always binds `127.0.0.1` only — there's no flag to bind anywhere else, since the threat model is "single trusted machine, bearer token," not "reachable over the network." Every concurrent client session gets its own independent browser (its own profile directory), same cost model as running multiple stdio `truewright mcp` processes. Stdio remains the default when `--http` isn't passed; nothing about existing stdio usage changes.
...
**Requirements and failure modes:** Windows only, and the session must be headed (`truewright mcp --headed`) — a headless renderer has no real OS window to receive input, and there's no macOS/Linux backend yet. Both cases fail with a clear tool error, never a silent fallback to CDP dispatch. Because it's real OS input, it briefly takes over your actual mouse cursor and window focus — expect the browser window to visibly pop to the foreground for the duration of the action, same as if you'd clicked it yourself.
...
```
truewright mcp                        # headless, managed chrome-headless-shell (auto-downloaded/cached)
truewright mcp --headed                # show the browser window (installed browser)
truewright mcp --browser installed     # headless, but always the installed browser (no shell download)
```
...
Produces `target/release/truewright.exe`.
...
```
truewright doctor                     # human-readable report; shell + installed browsers, headless
truewright doctor --headed            # show the browser windows (installed browsers only — shell is headless-only)
truewright doctor --browser installed # skip the managed shell, installed browsers only
truewright doctor --json              # machine-readable report for CI
```
...
Because caller flags are appended last, they override the built-in defaults under Chrome's last-flag-wins rule — including the headless mode. Headless full-Chrome runs default to the lightweight old `--headless`; pass `--chrome-arg=--headless=new` to opt into the heavier new headless mode.

### Default automation flags (headless)

Headless launches carry Chrome's automation-oriented default flag set, matching Playwright's, to shed background service processes and resident memory (biggest wins: `--disable-features=…`, `--disable-component-extensions-with-background-pages`, `--disable-breakpad`, `--disable-sync`, `--metrics-recording-only`, `--no-service-autorun`) — taking the Chrome process tree from ~15 down to ~10. Any of them can be overridden or removed via the three flag sources above.

### CDP transport (pipe vs WebSocket)

On Unix, `truewright` speaks CDP over a `--remote-debugging-pipe` (inherited fds 3/4) instead of a TCP loopback debugging port — no port to allocate or scan, and none of the "timed out attaching to browser devtools endpoint" flakiness a port scan hit in sandboxed/container environments. Windows uses the TCP DevTools WebSocket. Set `TRUEWRIGHT_CDP_TRANSPORT=websocket` to force the WebSocket path on Unix as an escape hatch.
...
## LLM provider config (`<data-dir>/truewright/config.toml`)

The foundation for `truewright`'s own agent harness (see Status above — the driving loop itself isn't shipped yet, but the provider/auth layer it's built on is usable standalone today via `truewright llm ping`). Config file resolution order: `--config <path>` → `TRUEWRIGHT_CONFIG` env var → project-local `./truewright.toml` → `<data-dir>/truewright/config.toml`. A missing file is a valid empty config — nothing here is required for the browser tools to work.
...
oauth_flow = "chatgpt"                  # auth via `truewright auth login chatgpt`, not a key
...
```
truewright llm ping driver             # resolves the "driver" role and sends one real completion
truewright auth login chatgpt          # PKCE browser login for a subscription-based provider
truewright auth status                 # lists stored logins and their expiry
truewright auth logout chatgpt         # deletes stored tokens for a flow
```

`truewright auth login`'s ChatGPT flow uses the same `auth.openai.com` PKCE flow and `chatgpt.com/backend-api/codex` backend OpenAI's own open-source Codex CLI uses — unofficial, interoperable-client territory (not a published integration surface), so it can drift if OpenAI changes it. `truewright` sends its own `originator` header rather than impersonating Codex's client identity.
...
Builds a Debian + Chromium image, runs `cargo test --workspace` and `truewright doctor --json` inside a container, and discards everything when it exits — nothing touches your host's browser or its profile directories. `crates/cdp`'s browser discovery works on both Windows (registry + `%LOCALAPPDATA%`) and Linux (`/usr/bin/chromium` etc. + `$XDG_DATA_HOME`/`~/.local/share`) for this reason.

### Forcing a specific browser binary (`TRUEWRIGHT_CHROME_PATH`)

Set `TRUEWRIGHT_CHROME_PATH` to an absolute path and browser discovery uses exactly that binary — skipping the registry/well-known-path search *and* the managed `chrome-headless-shell` for headless runs — or fails immediately if the path doesn't exist, rather than silently falling back to whatever else is installed. This exists for testing against a specific channel (e.g. Chrome Beta) regardless of where it happens to be installed:

```
bash docker/run-tests-chrome-beta.sh
```

Builds a second, Chrome-Beta-flavored test image (`docker/Dockerfile.chrome-beta`, installing `google-chrome-beta` from Google's own apt repo) and runs the full suite against it with `TRUEWRIGHT_CHROME_PATH` set. A weekly scheduled run of this lives at `.github/workflows/chrome-beta.yml`, to catch CDP protocol drift before it reaches Stable — **note:** this repository has no git remote configured yet, so that workflow's execution on GitHub's runners has been written and locally syntax-checked but not observed actually running; see `openspec/specs/browser-attach/spec.md` and the archived `weekly-chrome-beta-ci` change for the full story.
