# deterministic-init Specification

## Purpose
Deterministic page initialization: register JavaScript that runs before a page's own scripts (not after, unlike this project's other injected scripts), and a built-in seeded `Math.random` override built on that mechanism — so an app's own use of randomness during startup becomes reproducible run to run, the same way network-mocking makes an app's own network calls reproducible.
## Requirements
