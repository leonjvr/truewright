# same-origin-iframes Specification

## Purpose
Lets an agent see and interact with content inside same-origin iframes (rich-text editors, embedded widgets, app-internal micro-frontends) instead of a snapshot silently showing nothing where one exists, while being honest that a cross-origin iframe is a real boundary this project doesn't cross yet.
## Requirements
