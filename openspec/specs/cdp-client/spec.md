...
A minimal, hand-written Chrome DevTools Protocol client: one WebSocket connection, demuxed by `sessionId` in flatten mode, with typed structs for the commands/events ai-browser actually uses and a raw escape hatch for everything else.
