...
Decision #1 as originally written claimed the single-key-map step format (`- navigate: "url"`) was "serde's default enum representation." That was wrong for `serde_yaml` specifically, and the first integration test run against real, hand-written YAML caught it immediately (not a code-review guess): a `#[derive(Deserialize)]`'d `Step` enum with `#[serde(rename_all = "snake_case")]` parsed `- navigate: "url"` and failed with `invalid type: map, expected a YAML tag starting with '!'`. `serde_yaml` represents a newtype variant (a single-field tuple variant like `Navigate(String)`) using YAML's native tag syntax (`!navigate "url"`) by default -- a deliberate difference from `serde_json`, which has no tag concept and so falls back to `{"navigate": "url"}`. Struct variants would likely have worked as plain maps, but converting every step (including the simple single-string ones) to a struct variant just to dodge the newtype-tag behavior would have made the common case noisier (`- navigate: {url: "..."}` instead of `- navigate: "..."`) for no real benefit.

Fixed with a hand-written `Deserialize`/`Serialize` for `Step`: deserialize into a `HashMap<String, serde_yaml::Value>` (rejecting anything but exactly one key), dispatch on that key to the right variant, and deserialize the value into a small private payload struct for the multi-field steps (`type`/`wait_for`/`assert`). `Serialize` mirrors it with `serializer.serialize_map(Some(1))`. More code than a derive, but it's what actually delivers the human-writable, GitHub-Actions/Ansible-style format the whole feature exists for -- the derive's behavior was an implementation detail of the YAML crate, not something worth exposing to whoever writes a test script by hand. Re-verified: a hand-written script using the intended map syntax now parses and runs correctly, and export (which uses the same custom `Serialize`) round-trips through re-parsing without needing any special-casing.
